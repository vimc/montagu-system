...
function getEncodedToken(username = "test user name", expiry = new Date(now.getTime() + (60 * 60 * 1000))) {
...
test('can decode jwt token', () => {
    ...
    const mockInflateAppend = jest.fn(x => x + "inflated");
    const mockDecodeAppend = jest.fn(x => x + "decoded");


    const sut = new MontaguLogin(null, null, mockDecodeAppend, {inflate: mockInflateAppend});
...
test('can get user name from successful result', async () => {
    ...
    const result = await sut.getUserName();
...
test('sets packitLoginError if Packit auth fails after Montagu auth succeds', async () => {
    const encodedToken = getEncodedToken();
    ...
    const result = await sut.login("test email", "test password");
    expect(result.username).toBe("test user name");
    expect(result.packitLoginError).toBe("Montagu login succeeded, but Packit login failed.");

    //Expected mocks were called
    expect(mockPackitLogin).toHaveBeenCalledWith(encodedToken);
    expect(mockPackitSaveUser).not.toHaveBeenCalled();
});
